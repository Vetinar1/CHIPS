...
points = np.random.random_sample((60000, 4))
points[:,0] = points[:,0] * (max(nH) - min(nH)) + min(nH)
points[:,1] = points[:,1] * (max(T) - min(T)) + min(T)
points[:,2] = points[:,2] * (max(old) - min(old)) + min(old)
points[:,3] = points[:,3] * (max(SFR) - min(SFR)) + min(SFR)
...
folder = "random_gasoline_header2/"
os.mkdir(folder)
for filename in filenames:

...
# 6. Choose the number of random samples to draw at each redshift
sample_count = 10000
...
for z in np.arange(z_min, z_max, z_step):
    points = np.random.random_sample((sample_count, dims))
    # scale output of random_sample to cover parameter space. the two dicts have the limits at different locations
    for i, v in enumerate(param_space.values()):
        points[:,i] = points[:,i] * (v[1] - v[0]) + v[0]

    for j, v in enumerate(rad_param_space.values()):
        offset = len(param_space.keys())
        i = j + offset
        points[:,i] = points[:,i] * (v[1][1] - v[1][0]) + v[1][0]
    ...
    mergedata.to_csv(os.path.join(out_subfolder, f"z{z}.points"), index=False)
